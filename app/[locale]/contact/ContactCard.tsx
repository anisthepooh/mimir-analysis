...
const ContactCard = ({author}: {author: any}) => {
  return (
    <Card className="flex gap-4 p-8 max-w-sm">
    <Avatar>
      {author.image ? (
    ...
    <div className="">
      ...
      {author.phone && <div className="flex items-center pt-2 gap-2">
        <Phone size={12} className='text-muted-foreground' /> 
        <p className="text-sm">{author.phone}</p>
      </div>}
      {author.mail && <div className="flex items-center pt-2 gap-2">
        <MailIcon size={12} className='text-muted-foreground' /> 
        <p className="text-sm">{author.mail}</p>
      </div>}
      {author.link && <div className="flex items-center pt-2 gap-2">
