...
const TestimonialSection = ({hidden}: Props) => {
  ...
  return (
    <div id="testimonials" className="max-w-screen-xl mx-auto px-4 md:px-8 mt-32">
      <div className="max-w-2xl sm:text-center md:mx-auto">
        <h2 className="text-slate-800 text-3xl font-semibold sm:text-4xl">
              ...
              <Card key={idx}>
                <CardHeader className='pb-4'>
                  <div className="flex items-center gap-x-4">
                    <Avatar>
                      <AvatarImage src={item.avatar} className="w-14 h-14 object-cover rounded-full" alt={item.name} />
                    </Avatar>
                    <div>
                        <span className="block text-slate-800 font-semibold">{item.name}</span>
                        <span className="block text-slate-600 text-sm">{item.title}</span>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
